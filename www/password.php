...
$user = db_query($db, "SELECT * FROM users WHERE username='".$_SESSION['username']."'");
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  $row = db_query($db, "SELECT password FROM users WHERE username = '$username'");
  ...
  if(password_verify($_POST['current_pass'], $row['password']) || (isset($_SESSION["pass-reset"]) && $_SESSION["pass-reset"])){
    ...
    if($_POST['new_pass'] == $_POST['new_pass_confirm']){
      ...
      db_query_no_result($db, "UPDATE `users` SET password='$hash' WHERE username = '$username'");
  ...
  <body>
    ...
    <div class="container">
      <form class="form-horizontal" method="post">
        ...
        <!-- Form Name -->
        <center><h1 class="page-header"><b>Change password</b></h1></center>
    ...
    <?php include("src/html/footer.html"); require_once('../src/php/alert.php');?>
